...
export const DriverSignUp = () => {
    ...
    const initialValues = { firstname: "", lastname: "", email: "", password: "", checkbox: false, phone: "", dob: "", gender: "", licence: "", vehicle_name: "", vehicle_number: "" };
    ...
    async function emailNotExists(email) {
        const resp = await axios.post(`http://localhost:3050/DriverEmailCheck`, { email_id: email }).then((res) => {
            if (res.data.status) {
                if (!res.data.exists)
                    return 1;
                else
                ...
                setFormErrors({ ...formErrors, server: "Something's Wrong with the Server" });
                return 0
            }
        })
    ...
    async function createDriver() {
        console.log("Inside Create Driver");
        const resp = await axios.post(`http://localhost:3050/DriverCreate`, {
    ...
    const redirector = () => {
        let timer = setTimeout(() => { navigate("/DriverLogin") }, 10000);
        clearTimeout(timer);
        timer = setTimeout(() => { navigate("/DriverLogin") }, 5000);
    }
    useEffect(() => {
        ...
        <div>
            {!isFirstSubmit && (<div className='container formsContainer'>
                <form className='h-100 p-4 text-light' onSubmit={handleFirstSubmit}>
                    <h1 className="h3 mb-4 fw-normal">Earn with a Reliable Driver Platform!</h1>
                    ...
                    <p>Already have an account? Click to <Link to="/DriverLogin" className='text-light'>Log In</Link></p>
                </form>
            </div>)}
            ...
            {isFirstSubmit && !isSecondSubmit && (<div className='container formsContainer'>
                <form className='h-100 p-4 text-light' onSubmit={handleSubmit}>
                    ...
                    <div className="form-floating">
                        <input className="form-control text-light bg-dark" id="licence" placeholder="XXX-XXXXXXX" value={formValues.licence} onChange={handleChange} />
                        <label htmlFor="floatingInput">Licence</label>
                        <p className="text-warning text-start">
                            {formErrors.licence}
                        </p>
                    </div>
                    <div className="form-floating">
                        <input className="form-control text-light bg-dark" id="vehicle_name" placeholder="XXX-XXXXXXX" value={formValues.vehicle_name} onChange={handleChange} />
                    ...
                    <div className="form-floating">
                        <input className="form-control text-light bg-dark" id="vehicle_number" placeholder="XXX-XXXXXXX" value={formValues.vehicle_number} onChange={handleChange} />
            ...
            {showSuccessScreen && (<div className='container formsContainer text-light'>
                ...
                <p>You Will be redirected to <Link to="/DriverLogin" className='text-light'>Log In</Link> page in a moment.</p>
            </div>)}
        </div>
    )
}
